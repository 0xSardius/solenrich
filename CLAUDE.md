...
- **Slow endpoints settle BEFORE the handler (2026-09-20).** The stock `@x402/hono` order is verify → handler → settle; a Solana payment's blockhash expires in ~60s, so any handler slower than that loses the settlement (`transaction_simulation_failed`). `SETTLE_FIRST_ENDPOINTS` in `src/config.ts` lists the routes that use `src/lib/settle-first.ts` (verify → settle → handler). If you add an endpoint that can run past ~20s cold, add its key there, or it will settle only when upstreams are fast. A handler failure after settlement is a paid error: logged `[settle-first-loss]`, counted in `/metrics.process.settle_first_losses_today`. `src/lib/cache-warmer.ts` keeps the default-input result of `smart-money-flow` / `smart-money-trenches` warm while they are in demand (called in the last 2h).
