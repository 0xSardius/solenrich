...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  },
  {
    ...
    inputSchema: {
      mints: z.array(z.string()).min(1).max(25).optional().describe('Up to 25 reward coin mints; when given, the filters are ignored'),
      ...
      max_age_days: z.number().min(0).optional().describe('Maximum coin age in days'),
      max_market_cap_usd: z.number().min(0).optional().describe('Maximum market cap in USD'),
      paying_only: z.boolean().default(false).describe('Only coins that paid holders in the last 24h'),
      live_only: z.boolean().default(false).describe('Only coins that traded AND paid in the last 24h'),
      sort: z.enum(['yield7d', 'yield30d', 'rewardsUsd', 'volume24h', 'lastPayout', 'holders', 'priceChange24h']).default('volume24h').describe('Ranking key'),
      limit: z.number().int().min(1).max(25).default(25).describe('Coins to return (max 25)'),
    },
    handler: async (args) => invoke('stonk-yield-batch', {
      ...(args.mints?.length ? { mints: args.mints } : {}),
      ...(args.quote_mint ? { quote_mint: args.quote_mint } : {}),
      ...(args.category ? { category: args.category } : {}),
      ...(args.min_holders != null ? { min_holders: args.min_holders } : {}),
      ...(args.max_age_days != null ? { max_age_days: args.max_age_days } : {}),
      ...(args.max_market_cap_usd != null ? { max_market_cap_usd: args.max_market_cap_usd } : {}),
      paying_only: args.paying_only ?? false,
      live_only: args.live_only ?? false,
      sort: args.sort ?? 'volume24h',
      limit: args.limit ?? 25,
      format: 'llm',
    }),
  },
  {
    ...
    inputSchema: {
      ...
      max_age_days: z.number().min(0).optional().describe('Maximum coin age in days'),
      min_volume_24h_usd: z.number().min(0).optional().describe('Minimum 24h volume in USD'),
      max_market_cap_usd: z.number().min(0).optional().describe('Maximum market cap in USD'),
      paying_only: z.boolean().default(false).describe('Only coins that paid holders in the last 24h'),
      live_only: z.boolean().default(false).describe('Only coins that traded AND paid in the last 24h'),
      sort: z.enum(['yield7d', 'yield30d', 'rewardsUsd', 'volume24h', 'lastPayout', 'holders', 'priceChange24h']).default('volume24h').describe('Ranking key'),
    ...
    handler: async (args) => invoke('stonk-screener', {
      ...
      ...(args.max_age_days != null ? { max_age_days: args.max_age_days } : {}),
      ...(args.min_volume_24h_usd != null ? { min_volume_24h_usd: args.min_volume_24h_usd } : {}),
      ...(args.max_market_cap_usd != null ? { max_market_cap_usd: args.max_market_cap_usd } : {}),
      paying_only: args.paying_only ?? false,
      live_only: args.live_only ?? false,
      sort: args.sort ?? 'volume24h',
  ...
  },
  {
    ...
    inputSchema: {
      quote_mint: z.string().optional().describe('Only coins paired against this quote mint'),
      category: z.enum(['xstock', 'prestock', 'currency', 'leverage', 'solana', 'collectible', 'custom']).optional().describe('Quote category filter'),
      max_age_days: z.number().min(0).max(90).default(14).describe('Ignore coins older than this'),
      min_holders: z.number().int().min(0).default(25).describe('Ignore coins with fewer reward-receiving holders'),
      max_market_cap_usd: z.number().min(0).default(5_000_000).describe('Ignore coins above this market cap'),
      limit: z.number().int().min(1).max(50).default(15).describe('Rows to return'),
    },
    handler: async (args) => invoke('stonk-gems', {
      ...(args.quote_mint ? { quote_mint: args.quote_mint } : {}),
      ...(args.category ? { category: args.category } : {}),
      max_age_days: args.max_age_days ?? 14,
      min_holders: args.min_holders ?? 25,
      max_market_cap_usd: args.max_market_cap_usd ?? 5_000_000,
      limit: args.limit ?? 15,
      format: 'llm',
    }),
  },
  {
    name: 'stonk_launch_intel',
    title: 'StonkFun Launch Intel',
    ...
    inputSchema: {
      category: z.enum(['xstock', 'prestock', 'currency', 'leverage', 'solana', 'collectible', 'custom']).optional().describe('Quote category filter'),
      min_coins: z.number().int().min(1).default(5).describe('Only quotes with at least this many reward coins'),
      sort: z.enum(['demand', 'survival', 'volume', 'launches', 'paying']).default('demand').describe('Ranking key'),
      limit: z.number().int().min(1).max(100).default(20).describe('Rows to return'),
    },
    handler: async (args) => invoke('stonk-launch-intel', {
      ...(args.category ? { category: args.category } : {}),
      min_coins: args.min_coins ?? 5,
      sort: args.sort ?? 'demand',
      limit: args.limit ?? 20,
      format: 'llm',
    }),
  },
