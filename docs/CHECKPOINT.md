...
Working mode: one task at a time — brief with trade-offs first, wait for "go", verify live, one review per row.**

Done today, all verified on production: A1 settle-first + warmer (`ee53d49`, 4/4 slow endpoints settle),
A3 `/status` (`b9c4f33`), B1 counts + guard (`0d1055a`), B2 API root/robots/llms-full (`7a62aa6`),
B3 llms.txt stonk-first (`e529436`), B4 `/stonkfun` page (`c59ee9c`). A2 declined (manual smoke command in
the row). Findings: first **Base-network buyer** (`0x4c29…9494`, 10 × stonk-yield); **census 2.5× larger**
(19,916 coins, live share halved, ZEC shelf crowded out).

**Tomorrow's first choice (Sardius):** B5 (46 generated `/endpoints/{key}` pages — biggest SEO lever, needs
a shared stylesheet) vs. jumping to Block C1 (payout-staleness alerts on check-alerts, completes the stonk
suite). Either way: brief first. **Sardius side:** Search Console property (E5) → submit sitemap + request
re-index of `/` and `/stonkfun`; uptime checker on `/status`; post the ZEC draft (`local/outreach/`).
...
| # | Task | Size | Done when |
|---|---|---|---|
| A1 | `[x]` **DONE 2026-09-20 (`ee53d49`), verified live.** Settle-first middleware (`src/lib/settle-first.ts`: verify → settle → handler) on `SETTLE_FIRST_ENDPOINTS` = smart-money-flow, smart-money-trenches, trenches-scan, trenches-check; shares one `x402HTTPResourceServer` with the stock middleware. Paid-then-failed = `[settle-first-loss]` log + `/metrics.process.settle_first_losses_today`. Demand-driven warmer (`src/lib/cache-warmer.ts`) for smart-money-flow (10 min TTL) + smart-money-trenches (2 min) while called in the last 2h. Cold timings measured: smf 30s local/60s Railway, smt 27s, scan 26s, check 24s, runner-scan 2s; warm = ms. 11 unit tests. **Live paid run: 4/4 settled** (smf 200 in 2.0s, smt 33.4s, check 26.9s, scan 26.5s — the 26–33s handlers now settle because settlement happens before them); `/metrics`: settle_first_settled_today=4, losses=0, settlement_failures=0; warmer re-warmed smt in 26s after the paid call. One SolScout client-side 429 (its Helius RPC while building the payment tx) — not ours, re-run passed. CLAUDE.md rule added: any endpoint >20s cold goes in the set. | ½ day | ✅ Paid SolScout run settles `smart-money-flow`; verify→settle gap is 0 on all four. |
...
**A5 (Sardius, 2 min, found 2026-09-20 EOD):** `[ ]` Railway redeploys the API on EVERY push to main, including
docs-only and landing-only commits → a one-minute "index warming up" window per push (seen on `/status` after
the checkpoint commit). Set Railway **watch paths** to `src/**`, `package.json`, `bun.lock`, `Dockerfile`
(exclude `docs/`, `landing/`, `memory/`, `local/`) — or commit a `railway.json` with `watchPatterns`.
End-of-day verification 18:53 UTC: /status ok, 402 sweep 48/48, paid parse-transaction + stonk-quote settled.
...
| # | Task | Size | Done when |
|---|---|---|---|
| B1 | `[x]` **DONE 2026-09-20 (`0d1055a`), verified live.** 45 paid + 1 free, **44 MCP tools** (real count from `MCP_TOOLS`; earlier "45" was a loose grep). Fixed: landing og/twitter descriptions, JSON-LD, H2 (all said 38), two MCP mentions (36); docs.html meta (42) + og/twitter (33 — found by the guard, missed by hand); README MCP section (32). GitHub repo description already said 45. Guard: `test/copy-counts.test.ts` reads PRICING/FREE_ENDPOINTS/MCP_TOOLS and fails CI on any "<N> endpoints/tools" claim in landing/index.html, landing/docs.html, README.md that disagrees ("N endpoint tests" excluded). Live check after Vercel deploy: every tag says 45/44, zero stale claims on www + /docs. Google snippet catches up on re-crawl (Search Console, E5). | small | ✅ live pages verified; guard in CI. |
| B2 | `[x]` **DONE 2026-09-20 (`7a62aa6`), verified live.** Audit correction: the API root was the Lucid SDK's generated entrypoint page (indexable, no description/canonical), not a landing copy. Now: `GET /` with a browser/crawler Accept → 301 `https://www.solenrich.com/` (verified with a Googlebot UA+Accept); curl/fetch/no-Accept → JSON index of discovery surfaces (`src/lib/root.ts`, intercepted at the Bun fetch level like `/tasks`). API `robots.txt`: allow /docs, /llms*.txt, /openapi.json, /.well-known/, /status, /health; disallow /entrypoints/, /demo/, /mcp, /tasks, /metrics. `llms-full.txt` (`src/lib/llms-full.ts`): every endpoint with inputs (type/required/default/enum/range) + methodology + data sources from `buildDocs()` (hoisted out of the /docs handler) — 37 KB, 46 sections, live on api and via Vercel rewrite on www (38 KB, 200). Post-deploy: /docs, /status, /health, /openapi.json, /llms.txt unchanged; free 402 sweep 48/48. Tests: `test/discovery-surfaces.test.ts` (root negotiation, robots, llms-full covers every key), in CI. | small | ✅ all four surfaces verified on production. |
| B3 | `[x]` **DONE 2026-09-20 (`e529436`), verified live.** `src/lib/suites.ts` = the one presentation order (StonkFun → trenches → smart money → perps → core → discovery → temporal → composition → collectibles; entry call first in each). `src/lib/llms-txt.ts` builder: lead names the suites that sell + counts from code; "Start here" = stonk loop ($0.075) and trenches loop ($0.13) with prices from PRICING; suite headers with blurbs. `llms-full.txt` follows the same order, no leftover bucket. Live: api + www both serve the new copy; first link `stonk-pairs`; 402 sweep 48/48. `test/llms-order.test.ts` (every key in exactly one suite, order, once-each, loop totals) in CI; CLAUDE.md checklist item 4 names the suite map. | small | ✅ live on both hosts; stonk is the first group. |
| B4 | `[x]` **DONE 2026-09-20 (`c59ee9c`), live at https://www.solenrich.com/stonkfun** (200, title + H1 verified, nav link + sitemap entry live). Built on the **fresh 2026-09-20 census** (memory `project_stonk_census_2026_09_20.md`; script `local/scripts/census-stonk.ts`): 19,916 coins, 1,046 launches/day, 15.6% traded / 8.6% paid in 24h, 9.0% of >3d coins still trade, 585 coins ≥500 holders, 300 bps pays 12.0% vs 6.5%; ZEC shelf 87% → 7% traded; DOGE/XMR highest demand. Page: hero + dated tiles, what a reward coin is, three jobs, the four-call loop with **real production output** (ADOG/XMR: gems 97, quote PAYS $37.88/7d w/ youth caution, exit DERISK 0.445 — `local/scripts/b4-examples.json`, $0.075 spent), eight endpoint cards, quote table, 7-question FAQ (+ FAQPage JSON-LD), how to pay. Guard: page in `copy-counts` FILES (may carry no numeric claim). **Sardius:** Search Console re-index (E5) once the property exists; ZEC post draft in `local/outreach/zec-shelf-post-2026-09-20.md`. Numbers are static + dated: re-run the census weekly (E7) and edit the tiles. | 1 day | ✅ live; re-index = Sardius. |
